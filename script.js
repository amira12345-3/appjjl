...
function renderBossQ(ws, data, qIdx) {
  ...
  if (q.code) {
    // q.code is stored with &lt;/&gt; entities; decode to plain text first,
    // then let syntaxColor handle ALL highlighting safely.
    const plainCode = q.code
      .replace(/&lt;/g, '<')
      .replace(/&gt;/g, '>')
      .replace(/&amp;/g, '&')
      .replace(/&quot;/g, '"');
    qArea.innerHTML += `<div class="boss-q-code"><div class="code-block"><pre style="margin:0;white-space:pre-wrap;font-size:.82rem">${syntaxColor(plainCode)}</pre></div></div>`;
  }
...
function syntaxColor(code) {
  // Use null-byte placeholders to protect strings & comments
  // from being corrupted by subsequent keyword replacements.
  const preserved = [];
  const ph = (html) => { preserved.push(html); return `\x00${preserved.length - 1}\x00`; };

  let r = code;

  // 1. Protect // comments first
  r = r.replace(/\/\/.*/g, m => ph(`<span class="cmt">${m}</span>`));

  // 2. Protect string literals (real " in code; &quot; is NOT a Java string delimiter)
  r = r.replace(/"([^"\n]*)"/g, (_, inner) =>
    ph(`<span class="str">"${inner}"</span>`));

  // 3. Keywords / types (safe — no " left to re-match)
  r = r.replace(/\b(ArrayList|String|Integer|int|boolean|void|new|for|if|return|true|false|null)\b/g, m => {
    const types = ['ArrayList', 'String', 'Integer'];
    return types.includes(m)
      ? `<span class="typ">${m}</span>`
  ...
  // 5. Method calls
  r = r.replace(/\.(add|get|set|remove|size|indexOf|contains|equals|println)\b/g,
    m => `.<span class="mth">${m.slice(1)}</span>`);

  // 6. Restore protected tokens
  r = r.replace(/\x00(\d+)\x00/g, (_, i) => preserved[+i]);
